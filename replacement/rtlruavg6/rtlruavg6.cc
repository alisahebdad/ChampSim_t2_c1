...
void rtlruavg6::init_csv_file(const std::string& filename,
...
void rtlruavg6::add_csv_row(const std::string& filename,
...
rtlruavg6::rtlruavg6(CACHE* cache, long sets, long ways) : replacement(cache), NUM_WAY(ways), last_used_cycles(static_cast<std::size_t>(sets * ways), 0) {
  ...
  std::cout << "rtlruavg6 installed " << std::endl;
  clean_data();

}

long rtlruavg6::find_victim(uint32_t triggering_cpu, uint64_t instr_id, long set, const champsim::cache_block* current_set, champsim::address ip,
...
void rtlruavg6::replacement_cache_fill(uint32_t triggering_cpu, long set, long way, champsim::address full_addr, champsim::address ip, champsim::address victim_addr,
                                 access_type type)
{
...
void rtlruavg6::update_replacement_state(uint32_t triggering_cpu, long set, long way, champsim::address full_addr, champsim::address ip,
...
void rtlruavg6::cal_avg(long set,access_type type, uint8_t hit){
...
void rtlruavg6::cal_avg_nmru(long set,access_type type, uint8_t hit){
  long current_way = access_sq[set][0];
  long sum = 0;
...
void rtlruavg6::cal_avg_nmru_indicate1(long set,access_type type, uint8_t hit){
  long current_way = access_sq[set][0];
  long sum = 0;
