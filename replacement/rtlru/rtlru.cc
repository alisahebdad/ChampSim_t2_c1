...
{
  //std::cout << "find victim " << full_addr << " s:" << set << std::endl;
...
void rtlru::replacement_cache_fill(uint32_t triggering_cpu, long set, long way, champsim::address full_addr, champsim::address ip, champsim::address victim_addr,
                                access_type type)
{
  //std::cout << "replacement_cache_fill " <<  full_addr << " s:" << set << " " << way << std::endl;
                                   ...
                                   champsim::address victim_addr, access_type type, uint8_t hit)
{
  ...
  //std::cout << "update_replacement_state " << full_addr << " s:" << set << " " << way <<  this->myCache->NAME <<  std::endl;
  if (way != NUM_WAY || true){
