...
void rtlruavg5::init_csv_file(const std::string& filename,
...
void rtlruavg5::add_csv_row(const std::string& filename,
...
rtlruavg5::rtlruavg5(CACHE* cache) : rtlruavg5(cache, cache->NUM_SET, cache->NUM_WAY) {}

rtlruavg5::rtlruavg5(CACHE* cache, long sets, long ways) : replacement(cache), NUM_WAY(ways), last_used_cycles(static_cast<std::size_t>(sets * ways), 0) {
  extra_cycle_w = 0;
  ...
  std::cout << "rtlruavg5 installed " << std::endl;
  clean_data();

}

long rtlruavg5::find_victim(uint32_t triggering_cpu, uint64_t instr_id, long set, const champsim::cache_block* current_set, champsim::address ip,
...
void rtlruavg5::replacement_cache_fill(uint32_t triggering_cpu, long set, long way, champsim::address full_addr, champsim::address ip, champsim::address victim_addr,
                                 access_type type)
{
...
void rtlruavg5::update_replacement_state(uint32_t triggering_cpu, long set, long way, champsim::address full_addr, champsim::address ip,
...
long rtlruavg5::extra_cycle(){
  return extra_cycle_w;
}

void rtlruavg5::save_data(){
...
void rtlruavg5::cal_avg(long set,access_type type, uint8_t hit){
...
void rtlruavg5::cal_avg_nmru(long set,access_type type, uint8_t hit){
  long current_way = access_sq[set][0];
  long sum = 0;
...
void rtlruavg5::cal_avg_nmru_indicate1(long set,access_type type, uint8_t hit){
  long current_way = access_sq[set][0];
  long sum = 0;
