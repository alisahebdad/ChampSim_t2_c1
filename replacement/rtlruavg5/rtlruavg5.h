...
class rtlruavg5 : public champsim::modules::replacement
{
  long NUM_WAY;
  ...
  long extra_cycle_w;
public:
  explicit rtlruavg5(CACHE* cache);
  rtlruavg5(CACHE* cache, long sets, long ways);
  const int seq_num = 10;
  const int save_step = 1000*100;
  void add_csv_row(const std::string&,const std::vector<long long>&);
  void init_csv_file(const std::string&,const std::vector<std::string>&);
