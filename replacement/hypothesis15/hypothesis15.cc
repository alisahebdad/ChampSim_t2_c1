...
hypothesis15::hypothesis15(CACHE *cache)
    : hypothesis15(cache, cache->NUM_SET, cache->NUM_WAY) {
}
...
long hypothesis15::find_victim(uint32_t triggering_cpu, uint64_t instr_id,
                               long set,
    ...
    std::partial_sort(candidates, candidates + num_lru_candidates,
                      candidates + NUM_WAY, [&](long a, long b) {
                          return last_used_cycles[set * NUM_WAY + a] <
                                 last_used_cycles[set * NUM_WAY + b];
                      });
    ...
    for (auto it = hist.begin(); it != hist.end(); ++it) {
        ...
        if (streaming_cores[c]) {
            if (dist < min_tier1_dist) {
                ...
                if (last_used_cycles[set * NUM_WAY + w] <
                    last_used_cycles[set * NUM_WAY + best_tier1_way]) {
                    best_tier1_way = w;
                }
            }
        }
        ...
        if (inactive_cores[c]) {
            if (dist < min_tier2_dist) {
                ...
                if (last_used_cycles[set * NUM_WAY + w] <
                    last_used_cycles[set * NUM_WAY + best_tier2_way]) {
                    best_tier2_way = w;
                }
            }
        }
    ...
    if (best_tier2_way != -1 && min_tier2_dist < min_dist) {
        // Tier 2: Stalled cores. We only use them if they offer a strictly
        // better shift than the standard LRU pool, because taking from Tier 2
        // bypasses LRU.
...
void hypothesis15::replacement_cache_fill(uint32_t triggering_cpu, long set,
    ...
    // Update history
    history_buffers[superset_idx].push_back(
        HistEntry{triggering_cpu, local_set, static_cast<uint32_t>(way)});
    ...
    auto dist += (local_set > old_port) ? (local_set - old_port)
                                        : (old_port - local_set);
    total_shifts += dist;
...
void hypothesis15::update_replacement_state(uint32_t triggering_cpu, long set,
                                            long way,
