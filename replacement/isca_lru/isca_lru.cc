...
isca_lru::isca_lru(CACHE *cache)
    : isca_lru(cache, cache->NUM_SET, cache->NUM_WAY) {
}
...
long isca_lru::find_victim(uint32_t triggering_cpu, uint64_t instr_id, long set,
    ...
    // Identify the top K LRU candidates from the ORIGINAL set that caused the
    // miss
    long candidates[128];
...
void isca_lru::replacement_cache_fill(uint32_t triggering_cpu, long set,
...
void isca_lru::update_replacement_state(uint32_t triggering_cpu, long set,
                                        long way, champsim::address full_addr,
                                        champsim::address ip,
                                        champsim::address victim_addr,
