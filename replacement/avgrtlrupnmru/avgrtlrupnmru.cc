...
avgrtlrupnmru::avgrtlrupnmru(CACHE* cache) : avgrtlrupnmru(cache, cache->NUM_SET, cache->NUM_WAY) {
}

avgrtlrupnmru::avgrtlrupnmru(CACHE* cache, long sets, long ways) : replacement(cache), NUM_WAY(ways), last_used_cycles(static_cast<std::size_t>(sets * ways), 0) {
  std::cout << "avgrtlrupnmru protected installed \n" ;
...
long avgrtlrupnmru::find_victim(uint32_t triggering_cpu, uint64_t instr_id, long set, const champsim::cache_block* current_set, champsim::address ip,champsim::address full_addr, access_type type)
{
...
void avgrtlrupnmru::replacement_cache_fill(uint32_t triggering_cpu, long set, long way, champsim::address full_addr, champsim::address ip, champsim::address victim_addr,
...
void avgrtlrupnmru::update_replacement_state(uint32_t triggering_cpu, long set, long way, champsim::address full_addr, champsim::address ip,
                                   champsim::address victim_addr, access_type type, uint8_t hit)
{

  long int sum = 0;
  auto it = this->track[set].rbegin();
  it++;
  for (; it != this->track[set].rend(); ++it){
      //if (set == 0) std::cout << *it << " " ;
      sum += *it ;
  }
  
  long avg           = sum/(this->track[set].size()-1);
...
void avgrtlrupnmru::replacement_final_stats(){
  std::cout << "\n avgrtlrupnmru RTM Statistic \n\n";
...
long avgrtlrupnmru::extra_cycle(){
